...
:: Remove qt.conf generated by windeployqt — it causes incorrect text rendering in the app
if exist "%STAGING_DIR%\qt.conf" del /f "%STAGING_DIR%\qt.conf"
...
:: Both SCRITE_STAGING_DIR and SCRITE_VERSION_SUFFIX must be baked in at configure
:: time — CPACK_PACKAGE_FILE_NAME and CPACK_INSTALLED_DIRECTORIES are evaluated then.
echo Refreshing CMake configuration...
set "STAGING_FWD=%STAGING_DIR:\=/%"
cmake -B "%PROJECT_ROOT%\%BUILD_DIR%" -S "%PROJECT_ROOT%" ^
    "-DSCRITE_STAGING_DIR=%STAGING_FWD%" ^
    "-DSCRITE_VERSION_SUFFIX=%VERSION_SUFFIX%"
if errorlevel 1 goto :error

:: Step 11: Run CPack NSIS generator
:: Use a separate build dir so CPack's _CPack_Packages work folder does not
:: end up inside STAGING_DIR (which is what CPACK_INSTALLED_DIRECTORIES points to).
echo Creating NSIS installer with CPack...
set "CPACK_BUILD_DIR=%PROJECT_ROOT%\packaging\_cpack_build"
if exist "%CPACK_BUILD_DIR%" rmdir /s /q "%CPACK_BUILD_DIR%"
cpack --config "%PROJECT_ROOT%\%BUILD_DIR%\CPackConfig.cmake" -G NSIS -B "%CPACK_BUILD_DIR%" --verbose --debug
if errorlevel 1 goto :error
rmdir /s /q "%CPACK_BUILD_DIR%"
