...
export VERSION=0.9.4h-beta
mkdir Scrite-0.9.4h-beta.AppImage
cd Scrite-0.9.4h-beta.AppImage
mkdir bin
...
cd ../
linuxdeployqt Scrite-0.9.4h-beta.AppImage/share/applications/Scrite.desktop -appimage -qmldir=../../qml -verbose=2 -no-translations -no-copy-copyright-files
cd ../
