...
mkdir Scrite-0.9.3s-beta
mv Scrite.app Scrite-0.9.3s-beta
cp ../../images/dmgbackdrop.png dmgbackdrop.png
cp ../../appicon.icns Scrite.icns
sed "s/{{VERSION}}/Version 0.9.3s Beta/" dmgbackdrop.qml > dmgbackdropgen.qml
...
~/Utils/create-dmg/create-dmg \
  ...
  --hdiutil-verbose \
  "Scrite-0.9.3s-beta.dmg" \
  "Scrite-0.9.3s-beta/"
rm -f background.png
rm -f dmgbackdrop.png
rm -f Scrite.icns
mv Scrite-0.9.3s-beta/Scrite.app .
rm -fr Scrite-0.9.3s-beta
