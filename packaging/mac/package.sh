...
mkdir Scrite-0.9.9h-beta
mv Scrite.app Scrite-0.9.9h-beta
cp ../../images/dmgbackdrop.png dmgbackdrop.png
cp ../../appicon.icns Scrite.icns
sed "s/{{VERSION}}/Version 0.9.9h Beta/" dmgbackdrop.qml > dmgbackdropgen.qml
...
~/Utils/create-dmg/create-dmg \
  ...
  --hdiutil-verbose \
  "Scrite-0.9.9h-beta.dmg" \
  "Scrite-0.9.9h-beta/"
rm -f background.png
rm -f dmgbackdrop.png
rm -f Scrite.icns
mv Scrite-0.9.9h-beta/Scrite.app .
rm -fr Scrite-0.9.9h-beta

mv ../../../Release/Scrite.app.sym Scrite-0.9.9h-beta.app.sym
