...
mkdir Scrite-2.0.15
mv Scrite.app Scrite-2.0.15
cp ../../images/dmgbackdrop.png dmgbackdrop.png
cp ../../appicon.icns Scrite.icns
sed "s/{{VERSION}}/Version 2.0.15/" dmgbackdrop.qml > dmgbackdropgen.qml
...
sed "s/{{VERSION}}/2.0.15/" dmg_settings_tmpl.py > dmg_settings.py

dmgbuild -s dmg_settings.py "Scrite-2.0.15" Scrite-2.0.15.dmg

rm -f background.png
rm -f dmgbackdrop.png
rm -f Scrite.icns
mv Scrite-2.0.15/Scrite.app .
rm -fr Scrite-2.0.15

mv ../../../Release/Scrite.app.sym Scrite-2.0.15.app.sym
