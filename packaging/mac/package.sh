...
mkdir Scrite-0.9.3u-beta
mv Scrite.app Scrite-0.9.3u-beta
cp ../../images/dmgbackdrop.png dmgbackdrop.png
cp ../../appicon.icns Scrite.icns
sed "s/{{VERSION}}/Version 0.9.3u Beta/" dmgbackdrop.qml > dmgbackdropgen.qml
~/Qt/5.15.13/clang_64/bin/qmlscene dmgbackdropgen.qml
...
~/Utils/create-dmg/create-dmg \
  ...
  --hdiutil-verbose \
  "Scrite-0.9.3u-beta.dmg" \
  "Scrite-0.9.3u-beta/"
rm -f background.png
rm -f dmgbackdrop.png
rm -f Scrite.icns
mv Scrite-0.9.3u-beta/Scrite.app .
rm -fr Scrite-0.9.3u-beta
