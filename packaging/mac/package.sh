...
mkdir Scrite-0.9.5h-beta
mv Scrite.app Scrite-0.9.5h-beta
cp ../../images/dmgbackdrop.png dmgbackdrop.png
cp ../../appicon.icns Scrite.icns
sed "s/{{VERSION}}/Version 0.9.5h Beta/" dmgbackdrop.qml > dmgbackdropgen.qml
...
~/Utils/create-dmg/create-dmg \
  ...
  --hdiutil-verbose \
  "Scrite-0.9.5h-beta.dmg" \
  "Scrite-0.9.5h-beta/"
rm -f background.png
rm -f dmgbackdrop.png
rm -f Scrite.icns
mv Scrite-0.9.5h-beta/Scrite.app .
rm -fr Scrite-0.9.5h-beta
