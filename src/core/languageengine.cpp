...
{
    return { { (QObject *)this, lang, this->name(), this->name(), false } };
}
...
LanguageEngine *LanguageEngine::instance()
{
    ...
    if (!typesRegistered) {
        Application::log("LanguageEngine::instance(): Registering types");
...
void LanguageEngine::loadConfiguration()
{
    ...
    if (file.open(QFile::ReadOnly)) {
        ...
        if (!scriptFonts.isEmpty()) {
            const QMetaObject *charMetaObject = &QtChar::staticMetaObject;
            const QMetaEnum scriptEnum =
                    charMetaObject->enumerator(charMetaObject->indexOfEnumerator("Script"));
        ...
        const QJsonValue availableLanguagesConfig = config.value("available-languages");
        if (!availableLanguagesConfig.isNull())
            m_availableLanguages->fromJson(availableLanguagesConfig);
...
void LanguageEngine::saveConfiguration()
{
    if (!m_configFileName.isEmpty()) {
        ...
        if (file.open(QFile::WriteOnly)) {
            ...
            const QJsonValue availableLanguagesConfig = m_availableLanguages->toJson();
            if (!availableLanguagesConfig.isNull())
                config.insert("available-languages", availableLanguagesConfig);
...
QStringList LanguageEngine::scriptFontFamilies(QChar::Script script) const
{
    ...
    if (m_defaultScriptFontFamily.contains(script))
        includeFontFamilies({ m_defaultScriptFontFamily[script] });

    const QString safeDefault = m_defaultScriptFontFamily.value(QChar::Script_Unknown);
    QFontDatabase::WritingSystem writingSystem =
            scriptWritingSystemMap().value(script, QFontDatabase::Any);
    if (writingSystem == QFontDatabase::Any || writingSystem == QFontDatabase::Latin)
        includeFontFamilies({ safeDefault });
