...
qreal RulerItem::convert(qreal val, RulerItem::Unit from, RulerItem::Unit to) const
{
    ...
    auto unitToPixel = [=](qreal val, Unit unit) {
        switch(unit) {
        case Inch: return val * pixelsPerIn;
        case Centimeter: return val * pixelsPerCm;
        default: break;
        }
        return val;
    };
...
void RulerItem::paint(QPainter *painter)
{
    ...
    auto unitToPixel = [=](qreal val, Unit unit) {
        switch(unit) {
        case Inch: return val * pixelsPerIn * m_zoomLevel;
        case Centimeter: return val * pixelsPerCm * m_zoomLevel;
        default: break;
        }
    ...
    if(m_paragraphLeftMargin > m_leftMargin && m_paragraphRightMargin > m_rightMargin)
    {
        ...
        paragraphMarginRect.setRight( rect.right() - unitToPixel(m_paragraphRightMargin,m_marginsUnit) );
