        ...
        ListView {
            ...
            function filter(scene) {
                if(scene) {
                    ...
                    if(scene.heading.enabled) {
                        var ret = true
                        if(ret && _locTypeFilter.items.length > 0)
                            ret &= _locTypeFilter.items.contains(scene.heading.locationType)
                ...
                return _locTypeFilter.items.length === 0 && _momentFilter.items.length === 0 && _locFilter.items.length === 0
            }
            ...
            delegate: Item {
                ...
                VclCheckBox {
                    ...
                    visible: _sceneDelegate.screenplayElement.scene && _sceneListView.filter(_sceneDelegate.screenplayElement.scene)
                    text: {
                        var scene = _sceneDelegate.screenplayElement.scene
                        if(scene && scene.heading.enabled)
                            return "[" + _sceneDelegate.screenplayElement.resolvedSceneNumber + "] " + (scene && scene.heading.enabled ? scene.heading.text : "")
                        return "NO SCENE HEADING"
                    }
                    checked: _sceneListView.selectedSceneNumbers.indexOf(_sceneDelegate.screenplayElement.elementIndex) >= 0
                    enabled: _sceneDelegate.screenplayElement.scene

                    onToggled: _sceneListView.select(_sceneDelegate.screenplayElement.elementIndex, checked)
                }
            }
        }
    }
