...
void ScreenplayElement::evaluateSceneNumber(SceneNumber &number, EvaluationMode mode)
{
    if (m_userSceneNumber.isEmpty()) {
        if (m_scene != nullptr && m_scene->heading()->isEnabled())
            number.nextMajor();

        // No user scene number: auto-generate
        if (mode & EM_AutoGeneratedNumbers) {
            int sn = number.major;
            if (m_sceneNumber != sn) {
                m_sceneNumber = sn;
                emit sceneNumberChanged();
    ...
    // Has user scene number - check if it follows the pattern: <digits><letters>
    // Examples: "8A", "42B", "100AA" are valid
    // Examples: "B8", "8B8", "ABC" are NOT valid
    static const QRegularExpression hasLetterRegEx(QStringLiteral("^\\d+[A-Z]+$"));
    const bool hasLetter = hasLetterRegEx.match(m_userSceneNumber).hasMatch();
    bool updateUserSceneNumber = false;

    if (hasLetter) {
        // User scene number with letter (e.g., "10A")
        number.nextMinor();
        updateUserSceneNumber = (mode & EM_UserSceneNumberWithLetter);
    } else {
        // User scene number without letter (e.g., "42")
        number.nextMajor();
        updateUserSceneNumber = (mode & EM_UserSceneNumberWithoutLetter);
    }

    if (updateUserSceneNumber) {
...
void Screenplay::disconnectFromScreenplayElementSignals(ScreenplayElement *ptr)
{
    ...
    disconnect(ptr, &ScreenplayElement::sceneContentChanged, this,
               &Screenplay::onSceneContentChanged);
    disconnect(ptr, &ScreenplayElement::sceneHeadingChanged, this,
               &Screenplay::onSceneHeadingChanged);
    disconnect(ptr, &ScreenplayElement::sceneElementChanged, this,
               &Screenplay::onSceneElementChanged);
    ...
    for (ScreenplayElement *element : std::as_const(m_elements)) {
        ...
        if (element->elementType() == ScreenplayElement::SceneElementType) {
            ...
            if (scene->heading()->isEnabled()) {
                ++nrScenes;
                element->evaluateSceneNumber(sceneNumber,
                                             minorAlso
                                                     ? ScreenplayElement::EM_DefaultMode
                                                     : ScreenplayElement::EM_AutoGeneratedNumbers);
            }
        } else {
    ...
    while (it != end) {
        ...
        auto includeTrackItem = [&distinctTracks, trackName,
            ...
            for (ScreenplayTrack &distinctTrack : distinctTracks) {
                ...
                for (const ScreenplayTrackItem &existingTrackItem :
                     std::as_const(distinctTrack.items)) {
