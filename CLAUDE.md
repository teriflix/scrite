...
Scrite is an open-source desktop screenwriting application built with Qt/QML. It supports Windows, macOS, and Linux. The active development branch is `qt6`, which is an in-progress port from Qt 5.15.19 to Qt 6.8+.

## Build System

The project uses CMake 3.27+ with Qt 6.8+. Open the project in Qt Creator and build, or use the command line:
...
The `qt6` branch is an active migration from Qt 5.15 to Qt 6.8. Commits are prefixed with `[Qt6-Port]`. When editing QML, prefer Qt 6 APIs — avoid deprecated Qt 5 QML APIs. SystemPalette is used for theming colors.
